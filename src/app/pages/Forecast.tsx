...
const airQualityData: {
  [key: string]: { aqi: number; level: string; pollutant: string };
} = {
        ...
        if (data.data.forecast && data.data.forecast.daily) {
          ...
          if (dailyForecast.pm25 && dailyForecast.pm25.length > 1) {
            ...
            setTomorrowData({
              aqi: tomorrowAqi,
              level:
                typeof tomorrowAqi === "number" ? getAQILevel(tomorrowAqi) : "",
              pollutant: "PM2.5",
              available: true,
            });
          } else {
          ...
          if (data.data.forecast && data.data.forecast.daily) {
            ...
            if (dailyForecast.pm25 && dailyForecast.pm25.length > 1) {
              ...
              setTomorrowData({
                aqi: tomorrowAqi,
                level:
                  typeof tomorrowAqi === "number"
                    ? getAQILevel(tomorrowAqi)
                    : "",
                pollutant: "PM2.5",
                available: true,
              });
  ...
  const getHealthRecommendation = (level: string) => {
    switch (level) {
      case "Low":
        return {
          message:
            "Air quality is good tomorrow. Perfect for outdoor activities!",
      ...
      case "Moderate":
        return {
          message:
            "Air quality will be moderate tomorrow. Limit prolonged outdoor activity.",
      ...
      case "High":
        return {
          message:
            "Air quality will be unhealthy tomorrow for outdoor activity.",
        ...
        {tomorrowData.available ? (
          <>
            ...
            <div className="bg-white rounded-2xl p-8 mb-8 border-3 border-cyan-200">
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {/* Today */}
                <div
                  className={`rounded-2xl p-6 border-4 ${getBgColorForLevel(currentData.level)} relative`}
                >
                ...
                {/* Tomorrow */}
                <div
                  className={`rounded-2xl p-6 border-4 ${getBgColorForLevel(tomorrowData.level)} relative`}
                >
                  ...
                  <div className="mt-4 text-center">
                    <p className="text-lg text-gray-600 mb-2">
                      {tomorrow.toLocaleDateString("en-MY", {
                        weekday: "long",
                      })}
                    </p>
                    <div
                      className="text-7xl font-bold mb-4"
                      style={{ color: getColorForLevel(tomorrowData.level) }}
                    >
                      {tomorrowData.aqi}
                    </div>
                    <div
                      className="inline-block text-white px-8 py-3 rounded-full text-2xl font-bold shadow-lg"
                      style={{
                        backgroundColor: getColorForLevel(tomorrowData.level),
                      }}
                    >
              ...
              <div className="mt-6 text-center">
                {typeof tomorrowData.aqi === "number" &&
                typeof currentData.aqi === "number" &&
                tomorrowData.aqi < currentData.aqi ? (
                  <div className="inline-flex items-center gap-3 bg-green-100 border-2 border-green-400 text-green-900 px-8 py-4 rounded-2xl">
                    ...
                    <p className="text-2xl font-bold">
                      Air quality improving by{" "}
                      {currentData.aqi - tomorrowData.aqi} points
                    </p>
                  </div>
                ) : typeof tomorrowData.aqi === "number" &&
                  typeof currentData.aqi === "number" &&
                  ...
                  <div className="inline-flex items-center gap-3 bg-yellow-100 border-2 border-yellow-400 text-yellow-900 px-8 py-4 rounded-2xl">
                    ...
                    <p className="text-2xl font-bold">
                      Air quality worsening by{" "}
                      {tomorrowData.aqi - currentData.aqi} points
                    </p>
                  </div>
                ) : (
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-6">
                ...
                <div className="bg-sky-50 rounded-xl p-6 border-2 border-sky-200">
                  ...
                  <p className="text-xl text-sky-800">
                    {tomorrowHealthRec?.maxOutdoorDuration}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-r from-cyan-50 to-sky-50 rounded-xl p-6 border-2 border-cyan-200">
                ...
                <ul className="grid md:grid-cols-2 gap-3">
                  {tomorrowHealthRec?.activities.map((activity, index) => (
                    <li
                      key={index}
                      className="flex items-center gap-3 text-xl text-cyan-800 bg-white p-4 rounded-xl border-2 border-cyan-100"
                    >
          ...
          <div className="bg-white rounded-2xl p-10 border-3 border-gray-300 text-center">
            ...
            <p className="text-2xl text-gray-700 leading-relaxed mb-6">
              Air quality forecast for {currentLocation} is currently
              unavailable.
            </p>
            <p className="text-xl text-gray-600">
              Please check again later or contact local environmental
              authorities.
            </p>
          </div>
        )}
