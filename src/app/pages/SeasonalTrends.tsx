...
const calculateAQISubIndex = (concentration: number, pollutant: keyof typeof AQI_BREAKPOINTS): number => {
  const breakpoints = AQI_BREAKPOINTS[pollutant];
  
  console.log(`[${pollutant.toUpperCase()}] Input concentration:`, concentration);
  ...
  if (pollutant === 'no2' || pollutant === 'so2' || pollutant === 'o3') {
    ...
    console.log(`[${pollutant.toUpperCase()}] Converted from ${concentration} ppm to ${convertedConcentration} ppb`);
  }
  
  // Find the correct breakpoint range
  for (let i = 0; i < breakpoints.length; i++) {
    const bp = breakpoints[i];
    if (convertedConcentration >= bp.cLow && convertedConcentration <= bp.cHigh) {
      // Apply linear interpolation formula
      const aqi = ((bp.iHigh - bp.iLow) / (bp.cHigh - bp.cLow)) * (convertedConcentration - bp.cLow) + bp.iLow;
      const roundedAqi = Math.round(aqi);
      
      console.log(`[${pollutant.toUpperCase()}] Breakpoint range: ${bp.cLow}-${bp.cHigh} → AQI ${bp.iLow}-${bp.iHigh}`);
      console.log(`[${pollutant.toUpperCase()}] Formula: ((${bp.iHigh} - ${bp.iLow}) / (${bp.cHigh} - ${bp.cLow})) × (${convertedConcentration} - ${bp.cLow}) + ${bp.iLow}`);
  ...
  console.log(`[${pollutant.toUpperCase()}] Concentration ${convertedConcentration} exceeds all breakpoints, returning max AQI: 500`);
  return 500;
};

const seasonalAdvice: { [key: string]: { title: string; advice: string; precautions: string[] } } = {
  'Haze Season': {
    title: 'Haze Season Alert (Jul-Sep)',
    advice: 'Air quality is typically unhealthy during this period due to transboundary haze. Stay indoors and use air purifiers.',
    precautions: [
  ...
  'Northeast Monsoon': {
    ...
    advice: 'Generally better air quality due to rain. Good time for outdoor activities.',
    precautions: [
  ...
  'Southwest Monsoon': {
    ...
    advice: 'Air quality may vary. Monitor daily forecasts before outdoor activities.',
    precautions: [
...
export default function SeasonalTrends() {
  ...
  const [calculatedSeasonalTrends, setCalculatedSeasonalTrends] = useState(seasonalTrends);
  
  // Determine season for each month
  const getSeasonForMonth = (monthIndex: number): string => {
    if (monthIndex >= 0 && monthIndex <= 1 || monthIndex === 10 || monthIndex === 11) return 'Northeast Monsoon'; // Nov-Feb (10,11,0,1)
    ...
    if (monthIndex === 2 || monthIndex === 3 || monthIndex === 9) return 'Inter-monsoon'; // Mar-Apr, Oct (2,3,9)
  ...
  useEffect(() => {
    const fetchAndProcessData = async () => {
      ...
      try {
        ...
        const response = await fetch('https://api.data.gov.my/data-catalogue?id=air_pollution');
        ...
        if (data && Array.isArray(data)) {
          ...
          const normalizePollutantName = (pollutant: string): string => {
            ...
            return mapping[pollutant] || pollutant.toLowerCase().replace('.', '');
          };
          ...
          data.forEach((record: any, index: number) => {
            ...
            if (record.pollutant && record.concentration !== null && record.concentration !== undefined) {
              const normalizedPollutant = normalizePollutantName(record.pollutant);
              
              // Only log first few records to avoid console spam
              if (index < 10 || recordsProcessed < 20) {
                console.log(`Record ${index}: Date=${dateKey}, Pollutant="${record.pollutant}" → "${normalizedPollutant}", Concentration=${record.concentration}`);
              }
              
              // Insert into the correct pollutant array
              if (dailyData[dateKey][normalizedPollutant as keyof typeof dailyData[typeof dateKey]]) {
                dailyData[dateKey][normalizedPollutant as keyof typeof dailyData[typeof dateKey]].push(record.concentration);
                recordsProcessed++;
              } else {
                if (index < 10) {
                  console.warn(`Unknown pollutant key: ${normalizedPollutant} from ${record.pollutant}`);
                }
              }
            }
          });
          
          console.log(`Total records processed and grouped by date: ${recordsProcessed}`);
          ...
          Object.keys(dailyData).forEach((dateKey, index) => {
            ...
            const avgCO = pollutantData.co.length > 0 ? pollutantData.co.reduce((a, b) => a + b, 0) / pollutantData.co.length : 0;
            const avgNO2 = pollutantData.no2.length > 0 ? pollutantData.no2.reduce((a, b) => a + b, 0) / pollutantData.no2.length : 0;
            const avgPM25 = pollutantData.pm25.length > 0 ? pollutantData.pm25.reduce((a, b) => a + b, 0) / pollutantData.pm25.length : 0;
            const avgPM10 = pollutantData.pm10.length > 0 ? pollutantData.pm10.reduce((a, b) => a + b, 0) / pollutantData.pm10.length : 0;
            const avgSO2 = pollutantData.so2.length > 0 ? pollutantData.so2.reduce((a, b) => a + b, 0) / pollutantData.so2.length : 0;
            const avgO3 = pollutantData.o3.length > 0 ? pollutantData.o3.reduce((a, b) => a + b, 0) / pollutantData.o3.length : 0;
            ...
            const aqiPM25 = avgPM25 > 0 ? calculateAQISubIndex(avgPM25, 'pm25') : 0;
            const aqiPM10 = avgPM10 > 0 ? calculateAQISubIndex(avgPM10, 'pm10') : 0;
            ...
            const maxAQI = Math.max(aqiCO, aqiNO2, aqiPM25, aqiPM10, aqiSO2, aqiO3);
            const dominantPollutant = Object.keys(pollutantAQIs).find(
              key => pollutantAQIs[key as keyof typeof pollutantAQIs] === maxAQI
            ) || 'PM2.5';
            
            // Log first few days for verification
            if (index < 5) {
              console.log(`Date: ${dateKey}, Daily AQI: ${maxAQI}, Dominant: ${dominantPollutant}, AQIs:`, pollutantAQIs);
            }
          ...
          console.log(`Total daily AQI calculations completed: ${dailyAQIResults.length}`);
  ...
  const selectedMonthData = selectedMonth ? calculatedSeasonalTrends.find(t => t.month === selectedMonth) : null;
  const selectedAdvice = selectedMonthData ? seasonalAdvice[selectedMonthData.season] : null;
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="bg-white rounded-3xl shadow-2xl p-10 mb-8 border-4 border-orange-200">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-5xl font-bold text-gray-900">Seasonal Patterns</h1>
        ...
        <div className="bg-white rounded-2xl p-8 mb-8 border-3 border-orange-200">
          ...
          <ResponsiveContainer width="100%" height={400}>
            <AreaChart data={calculatedSeasonalTrends}>
              <defs>
                <linearGradient id="colorAqi" x1="0" y1="0" x2="0" y2="1">
                  <stop key="start" offset="5%" stopColor="#f97316" stopOpacity={0.8}/>
                  <stop key="end" offset="95%" stopColor="#f97316" stopOpacity={0.1}/>
                </linearGradient>
              </defs>
              ...
              <YAxis 
                tick={{ fontSize: 18, fill: '#374151', fontWeight: 600 }} 
                stroke="#9ca3af"
                label={{ value: 'AQI Level', angle: -90, position: 'insideLeft', style: { fontSize: 20, fill: '#374151', fontWeight: 700 } }}
              />
              <Tooltip 
                contentStyle={{ 
        ...
        <div className="bg-white rounded-2xl p-8 mb-8 border-3 border-orange-200">
          ...
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {calculatedSeasonalTrends.map(trend => {
              const isHighRisk = trend.aqi > 100;
              const bgColor = trend.aqi > 100 ? 'bg-red-100 border-red-400' : 
                             trend.aqi > 50 ? 'bg-yellow-100 border-yellow-400' : 
              ...
              return (
                <button
                  ...
                  className={`p-5 border-3 rounded-xl transition-all text-center ${bgColor} ${
                    isSelected ? 'ring-4 ring-orange-400 shadow-2xl scale-110' : 'hover:shadow-lg hover:scale-105'
                  }`}
                >
                  <div className="text-xl font-bold text-gray-900 mb-2">{trend.month}</div>
                  <div className="text-4xl font-bold text-gray-900 mb-2">{trend.aqi}</div>
                  {isHighRisk && (
        ...
        {selectedMonthData && selectedAdvice ? (
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border-4 border-blue-400 rounded-2xl p-8">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div 
                    ...
                    style={{ 
                      backgroundColor: selectedMonthData.aqi > 100 ? '#ef4444' : 
                                     selectedMonthData.aqi > 50 ? '#f59e0b' : '#10b981'
                    }}
                  >
                    {selectedMonthData.aqi}
                  </div>
                </div>
                <h3 className="text-3xl text-blue-900 font-bold mb-3">{selectedAdvice.title}</h3>
                <p className="text-2xl text-blue-800 leading-relaxed mb-6">{selectedAdvice.advice}</p>
              </div>
            </div>
            
            <div className="bg-white rounded-xl p-8 border-3 border-blue-300 mb-6">
              <p className="text-3xl font-bold text-blue-900 mb-4">🛡️ Health Precautions:</p>
              <ul className="space-y-3">
                {selectedAdvice.precautions.map((precaution, index) => (
                  <li key={index} className="flex items-start gap-3 text-2xl text-blue-800">
            ...
            {selectedMonthData.season === 'Haze Season' && (
              <div className="bg-red-50 rounded-xl p-8 border-3 border-red-400">
                <p className="text-3xl font-bold text-red-900 mb-3">⚠️ Special Alert for Elderly:</p>
                <p className="text-2xl text-red-800 leading-relaxed">
                  During haze season, elderly persons and those with respiratory conditions should stay indoors as much as possible. 
                  Keep N95 masks ready and use air purifiers. Consult your doctor if you experience breathing difficulties.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="bg-gradient-to-r from-orange-50 to-yellow-50 border-3 border-orange-300 rounded-2xl p-10 text-center">
            ...
            <p className="text-3xl text-orange-900 font-bold mb-3">Plan Ahead for Better Health</p>
            <p className="text-2xl text-orange-800 leading-relaxed">
              Click on any month above to see specific health advice and precautions for that seasonal period.
              This helps you prepare for high-risk pollution seasons like the haze period.
            </p>
          </div>
        )}
        ...
        <div className="fixed bottom-4 right-4 bg-white px-4 py-2 rounded-lg shadow-lg border-2 border-gray-300">
          <p className="text-sm font-semibold">
            API: {apiStatus === 'loading' ? '⏳ Processing historical data...' : apiStatus === 'success' ? '✓ Data calculated' : '✗ Using default trends'}
          </p>
        </div>
      </div>
    </div>
