...
export default function CompareLocations() {
  ...
  const [location2, setLocation2] = useState("Select a city");
  ...
  const getMainPollutant = (apiData: any): string => {
    ...
    const getVal = (p: any): number | undefined => {
      if (p === undefined || p === null) return undefined;
      if (typeof p === "number") return p;
      if (typeof p === "object" && "v" in p && typeof p.v === "number")
        return p.v;
      return undefined;
    };

    const pm25 = getVal(pollutants.pm25);
    if (pm25 !== undefined) pollutantValues["PM2.5"] = pm25;
    const pm10 = getVal(pollutants.pm10);
    if (pm10 !== undefined) pollutantValues["PM10"] = pm10;
    const o3 = getVal(pollutants.o3);
    if (o3 !== undefined) pollutantValues["O3"] = o3;
    const no2 = getVal(pollutants.no2);
    if (no2 !== undefined) pollutantValues["NO2"] = no2;
    const so2 = getVal(pollutants.so2);
    if (so2 !== undefined) pollutantValues["SO2"] = so2;
    const co = getVal(pollutants.co);
    if (co !== undefined) pollutantValues["CO"] = co;
  ...
  const handleLocation1Change = async (
  ...
  const handleLocation2Change = async (
    e: React.ChangeEvent<HTMLSelectElement>,
  ) => {
    const selectedLocationName = e.target.value;
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        console.log(
          "CompareLocations Initial API call (geolocation) for Location 1",
        );
        console.log("Full API Response:", data);
        ...
        if (data.status === "ok" && data.data) {
          ...
          const location2Station = malaysiaCities.find(
            (station) => station.name === "Penang",
          );
          if (location2Station) {
            ...
            console.log(
              "CompareLocations Initial API call for Location 2 (Penang)",
            );
            console.log("Full API Response:", data2);

            if (data2.status === "ok" && data2.data) {
              const apiAqi2 =
                data2.data.aqi !== undefined ? data2.data.aqi : "0000";
              ...
              const matchingStation2 = malaysiaCities.find(
                (station) => station.uid === apiUid2,
              );

              if (matchingStation2) {
                console.log(
                  "Location 2 Matching station found:",
                  matchingStation2.name,
                );
  ...
  const getBetterLocation = (): string => {
    ...
    if (typeof data1.aqi === "string" || typeof data2.aqi === "string") {
      return "unknown";
    }
    return "equal";
  };
  ...
  const getComparisonSpeechText = (): string => {
    ...
    if (betterLocation !== "equal" && betterLocation !== "unknown") {
      comparisonText += `Comparing air quality between ${location1} and ${location2}. `;
    ...
    if (betterLocation === "unknown") {
      comparisonText += `Unable to determine air quality comparison. Select a city for both locations to compare their air quality levels. `;
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-rose-50 p-8">
      <div className="max-w-7xl mx-auto">
        ...
        {/* Winner Banner */}
        {betterLocation !== "equal" && betterLocation !== "unknown" && (
          <div className="bg-gradient-to-r from-green-100 to-emerald-100 border-4 border-green-400 rounded-2xl p-8 mb-8 text-center">
            ...
            <p className="text-2xl text-green-800">
              {betterLocation} has cleaner air with an AQI of{" "}
              {betterLocation === location1 ? data1.aqi : data2.aqi} compared to{" "}
              {betterLocation === location1 ? location2 : location1}'s{" "}
              {betterLocation === location1 ? data2.aqi : data1.aqi}
            </p>
          </div>
        )}
        ...
        {betterLocation === "unknown" && (
          <div className="bg-gradient-to-r from-blue-100 to-cyan-100 border-4 border-blue-400 rounded-2xl p-8 mb-8 text-center">
            <AlertTriangle className="w-16 h-16 text-blue-600 mx-auto mb-4" />
            <h2 className="text-4xl font-bold text-blue-900 mb-3">
              Unable to determine air quality comparison
            </h2>
            <p className="text-2xl text-blue-800">
              Select a city for both locations to compare their air quality
              levels
            </p>
          </div>
        )}
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {/* Location 1 Card */}
          <div
            className={`rounded-3xl shadow-xl p-8 border-4 ${getBgColorForLevel(data1.level)} ${betterLocation === location1 ? "ring-8 ring-green-400" : ""}`}
          >
            <div className="text-center mb-8">
              ...
              <div
                ...
                style={{ color: getColorForLevel(data1.level) }}
              >
                {data1.aqi}
              </div>
              <div
              ...
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xl text-gray-800 mb-2">
                    <strong>😷 Mask:</strong> {advice1.mask}
                  </p>
                  <p className="text-xl text-gray-800">
                    <strong>🏃 Outdoor Activity:</strong> {advice1.outdoor}
                  </p>
                </div>
              </div>
            </div>
          ...
          {/* Location 2 Card */}
          <div
            className={`rounded-3xl shadow-xl p-8 border-4 ${getBgColorForLevel(data2.level)} ${betterLocation === location2 ? "ring-8 ring-green-400" : ""}`}
          >
            <div className="text-center mb-8">
              <h3 className="text-4xl font-bold text-gray-900 mb-4">
                {location2}
              ...
              <div
                className="text-8xl font-bold mb-4"
                style={{ color: getColorForLevel(data2.level) }}
              >
                {data2.aqi}
              </div>
              <div
            ...
            <div className="mt-6 pt-6 border-t-3 border-gray-300">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xl text-gray-800 mb-2">
                    <strong>😷 Mask:</strong> {advice2.mask}
                  </p>
                  <p className="text-xl text-gray-800">
                    <strong>🏃 Outdoor Activity:</strong> {advice2.outdoor}
                  </p>
                </div>
              </div>
            </div>
            ...
            <table className="w-full text-xl">
              <thead>
                <tr className="border-b-3 border-gray-300">
                  <th className="py-4 px-6 text-left text-2xl font-bold text-gray-900">
                    Metric
                  </th>
                  <th className="py-4 px-6 text-center text-2xl font-bold text-gray-900">
                    {location1}
                  </th>
                  <th className="py-4 px-6 text-center text-2xl font-bold text-gray-900">
                    {location2}
