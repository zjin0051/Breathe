...
export default function SafeAreas() {
  ...
  useEffect(() => {
    ...
    const fetchInitialData = async () => {
      ...
      try {
        ...
        console.log('SafeAreas Fetching Malaysia stations from map bounds API...');
        const mapBoundsUrl = 'https://api.waqi.info/v2/map/bounds?latlng=0.85,99.60,7.60,119.30&networks=all&token=401c2a6d614319cdb1b93d1274b9c4e4894d6dc2';
        ...
        if (mapBoundsData.status === 'ok' && mapBoundsData.data) {
          ...
          mapBoundsData.data.forEach((station: any) => {
            ...
            const malayStation = malaysiaCities.find(city => city.uid === station.uid);
  ...
  const stationsDataSource = Object.keys(malaysiaStationsData).length > 0 ? malaysiaStationsData : airQualityData;
  ...
  const safeZones = Object.keys(stationsDataSource)
    ...
    .map(city => ({
      name: city,
      distance: calculateDistance(stationsDataSource[city].lat, stationsDataSource[city].lng),
      aqi: stationsDataSource[city].aqi,
      level: stationsDataSource[city].level,
      pollutant: stationsDataSource[city].pollutant
    }))
    .sort((a, b) => sortBy === 'distance' ? a.distance - b.distance : a.aqi - b.aqi);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="bg-white rounded-3xl shadow-2xl p-10 mb-8 border-4 border-green-200">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-5xl font-bold text-gray-900">Find Safe Areas</h1>
        ...
        <div className="bg-white rounded-2xl p-8 mb-8 border-3 border-green-200">
          ...
          <div className="mt-6 bg-gray-50 rounded-xl p-6 border-2 border-gray-300">
            <p className="text-xl text-gray-700 mb-2">Your current air quality:</p>
            <div className="flex items-center gap-4">
              <div className="text-5xl font-bold" style={{ color: getColorForLevel(currentData.level) }}>
                {currentData.aqi}
              </div>
              <div 
        ...
        {safeZones.length > 0 ? (
          <>
            <div className="bg-green-100 border-3 border-green-400 rounded-2xl p-8 mb-8">
              ...
              <p className="text-2xl text-green-800 leading-relaxed">
                We found {safeZones.length} location{safeZones.length > 1 ? 's' : ''} with better air quality than {currentLocation}.
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-8 mb-8 border-3 border-blue-200">
              ...
              <select
                value={sortBy}
                onChange={(e) => setSortBy(e.target.value as 'distance' | 'aqi')}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {safeZones.map((zone, index) => (
                ...
                >
                  ...
                  <h3 className="text-3xl font-bold text-gray-900 mb-6">{zone.name}</h3>
                  
                  <div className="space-y-4">
                    <div className="bg-white rounded-xl p-6 border-2 border-gray-300">
                      <p className="text-lg text-gray-600 mb-2">Distance from you</p>
                      <p className="text-3xl font-bold text-gray-900">{zone.distance} km</p>
                    </div>
                    
                    <div className="bg-white rounded-xl p-6 border-2 border-gray-300">
                      <p className="text-lg text-gray-600 mb-2">Air Quality Index</p>
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`text-xl font-bold ${getTextColorForLevel(zone.level)}`}>
                          ({zone.level} Risk)
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl p-6 border-2 border-gray-300">
                      ...
                      <p className="text-2xl font-bold text-gray-900">{zone.pollutant}</p>
                    </div>
                  </div>
                  ...
                  <div className="mt-6 pt-6 border-t-2 border-gray-300">
                    <div className="flex items-center gap-2 text-lg text-gray-600">
                      ...
                      <span>Updated: {new Date().toLocaleTimeString('en-MY', { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div className="bg-yellow-50 border-3 border-yellow-400 rounded-2xl p-10 text-center">
            ...
            <p className="text-2xl text-yellow-800 leading-relaxed mb-6">
              Air quality in nearby areas is similar or worse than {currentLocation}.
            </p>
