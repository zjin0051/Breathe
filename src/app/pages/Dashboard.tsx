...
import { GlobalSpeechControls } from '../components/GlobalSpeechControls';
...
const airQualityData: { [key: string]: { aqi: number; level: string; pollutant: string; lat: number; lng: number } } = {
  'Kuala Lumpur': { aqi: 68, level: 'Moderate', pollutant: 'PM2.5', lat: 3.1390, lng: 101.6869 },
  'Penang': { aqi: 42, level: 'Low', pollutant: 'PM10', lat: 5.4164, lng: 100.3327 },
  'Johor Bahru': { aqi: 85, level: 'Moderate', pollutant: 'PM2.5', lat: 1.4927, lng: 103.7414 },
  'Ipoh': { aqi: 38, level: 'Low', pollutant: 'PM10', lat: 4.5975, lng: 101.0901 },
  'Kuching': { aqi: 125, level: 'High', pollutant: 'PM2.5', lat: 1.5535, lng: 110.3593 },
  'Kota Kinabalu': { aqi: 35, level: 'Low', pollutant: 'O3', lat: 5.9804, lng: 116.0735 },
  'Melaka': { aqi: 55, level: 'Moderate', pollutant: 'PM10', lat: 2.1896, lng: 102.2501 },
  'Shah Alam': { aqi: 45, level: 'Low', pollutant: 'PM2.5', lat: 3.0733, lng: 101.5185 },
  'Petaling Jaya': { aqi: 52, level: 'Moderate', pollutant: 'PM2.5', lat: 3.1073, lng: 101.6067 },
  'Klang': { aqi: 48, level: 'Low', pollutant: 'PM2.5', lat: 3.0333, lng: 101.4500 },
};
...
const tomorrowForecastData: { [key: string]: { aqi: number; level: string; pollutant: string; available: boolean } } = {
  ...
  'Melaka': { aqi: 0, level: '', pollutant: '', available: false }, // No forecast available
...
export default function Dashboard() {
  ...
  const [comparisonCities, setComparisonCities] = useState<string[]>(['Kuala Lumpur', 'Penang', 'Kuching']);
  ...
  const getMainPollutant = (apiData: any): string => {
    ...
    if (pollutants.so2?.v !== undefined) pollutantValues['SO2'] = pollutants.so2.v;
  ...
  const fetchAirQualityData = async (locationName: string) => {
    ...
    const selectedStation = malaysiaCities.find(station => station.name === locationName);
  ...
  const getHealthRecommendation = (level: string) => {
    switch (level) {
      ...
      case 'Moderate':
        return {
          message: 'Air quality is moderate today. Limit prolonged outdoor activity.',
  ...
  const selectedMonthData = selectedMonth ? seasonalTrends.find(t => t.month === selectedMonth) : null;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="bg-white rounded-3xl shadow-2xl p-8 md:p-12 mb-8 border-4 border-indigo-200">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div>
              <h1 className="text-4xl md:text-6xl font-bold text-gray-900">Breathe</h1>
          ...
          <p className="text-xl md:text-2xl text-gray-700 leading-relaxed mt-4">
            Helping you plan safer outdoor activities with real-time air quality insights.
          </p>
          ...
          <div className="bg-red-600 text-white rounded-3xl shadow-2xl p-8 md:p-10 mb-8 border-4 border-red-700 animate-pulse">
            <div className="flex items-start gap-4 mb-6">
              ...
              <div className="flex-1">
                <h2 className="text-3xl md:text-4xl font-bold mb-3">⚠️ Air Quality Alert</h2>
                ...
                <div className="bg-red-700 rounded-2xl p-6 mb-4">
                  <div className="grid md:grid-cols-2 gap-6 text-xl">
                    <div>
                      ...
                      <p className="text-2xl">{currentData.aqi} - {currentData.level} Risk</p>
                    </div>
                    <div>
                      <p className="font-bold mb-2 flex items-center gap-2">
                ...
                <div className="bg-white text-red-900 rounded-2xl p-6">
                  <p className="text-xl font-bold mb-3">⚕️ Elderly Health Advisory:</p>
                  <p className="text-xl leading-relaxed">{healthRec.familyAdvice}</p>
                </div>
              </div>
              <button 
        ...
        <div className="grid lg:grid-cols-3 gap-8 mb-8">
          ...
          <div className="lg:col-span-2">
            <div className={`rounded-3xl shadow-2xl p-8 md:p-10 border-4 ${getBgColorForLevel(currentData.level)}`}>
              ...
              <div className="text-center mb-8">
                <div className="text-9xl font-bold mb-4" style={{ color: getColorForLevel(currentData.level) }}>
                  {currentData.aqi}
                </div>
                <div 
                ...
                <div className="flex items-center justify-center gap-3 mt-4">
                  ...
                  <p className="text-2xl text-gray-700">
                    Main Pollutant: <span className="font-bold">{currentData.pollutant}</span>
                  </p>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl p-8 border-3 border-gray-300 mb-6">
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-gray-900 mb-3">Health Advisory</h3>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="bg-indigo-50 rounded-xl p-6 border-2 border-indigo-200">
                    <p className="text-xl font-bold text-indigo-900 mb-2">😷 Mask Advice:</p>
                    <p className="text-xl text-indigo-800">{healthRec.maskAdvice}</p>
                  </div>
                  <div className="bg-purple-50 rounded-xl p-6 border-2 border-purple-200">
                    <p className="text-xl font-bold text-purple-900 mb-2">⏱️ Max Outdoor Time:</p>
                    <p className="text-xl text-purple-800">{healthRec.maxOutdoorDuration}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-2xl p-8 border-3 border-gray-300 mb-6">
                <h3 className="text-2xl font-bold text-gray-900 mb-4">✅ Recommended Activities:</h3>
                <ul className="grid md:grid-cols-2 gap-3">
                  {healthRec.activities.map((activity, index) => (
                    <li key={index} className="flex items-center gap-3 text-xl text-gray-700 bg-gray-50 p-4 rounded-xl">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl shadow-xl p-8 border-4 border-blue-200">
              ...
              <div className="space-y-4">
                ...
                <div className="bg-green-50 rounded-xl p-6 border-2 border-green-200">
                  <p className="text-lg text-green-700 mb-1">Air Quality Index</p>
                  <p className="text-3xl font-bold text-green-900">{currentData.aqi}</p>
                </div>
                <div className="bg-purple-50 rounded-xl p-6 border-2 border-purple-200">
                  <p className="text-lg text-purple-700 mb-1">Status</p>
                  <p className="text-2xl font-bold text-purple-900">{currentData.level} Risk</p>
                </div>
                <div className="bg-orange-50 rounded-xl p-6 border-2 border-orange-200">
                  <p className="text-lg text-orange-700 mb-1">Main Pollutant</p>
                  <p className="text-2xl font-bold text-orange-900">{currentData.pollutant}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gradient-to-br from-indigo-100 to-purple-100 rounded-2xl p-6 border-3 border-indigo-300">
              <div className="flex items-center gap-3 mb-2">
                ...
                <p className="text-lg font-semibold text-indigo-900">Last Updated</p>
              </div>
